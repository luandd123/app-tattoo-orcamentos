  ...
  return (
    <Shell>
      <div className="flex items-center justify-between flex-wrap gap-4 mb-7">
        ...
        <Link href="/orcamentos/novo" className="btn btn-primary w-full sm:w-auto">
          + Novo orçamento
        </Link>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {STATUS_LIST.map((s) => (
            <Link
              ...
              className="card p-5 hover:-translate-y-0.5 transition relative overflow-hidden flex flex-col justify-between min-h-[112px]"
            >
              <span
              ...
              <div className="text-muted text-[12.5px] mt-2 font-medium leading-snug pr-5">{s.label}</div>
            </Link>
          ))}
        </div>
      )}
